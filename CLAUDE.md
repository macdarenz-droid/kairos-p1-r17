...
14. No ZIP candidates, no helper or bridge workflows, no dated or versioned file names, no per-task report files. The record is the commit, one Relay message and one `LOG.md` line.
    Delete a file only when the task names it (owner rule D16). Your `LOG.md` line then lists each deleted path with its backup, e.g. `restore: git checkout <commit-before> -- app/src/…`. Git history is the backup, so never force-push or rewrite it.
