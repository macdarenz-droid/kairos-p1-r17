...
export async function restoreAndVerifyKairosDatabase(
  ...
  const [metadata, trades, tradePlans, tradeExecutions, tradeFees, savedAnalyses, savedTimeAssistedSnapshots, tradeDiscipline] = await db.transaction(
    'r',
  ...
  return Object.freeze({
    ...
    restoredSavedAnalysisRecords: replacement.restoredSavedAnalysisRecords,
    ...
    reloadedTotalRecords: backupMetadata.length + trades.length + tradePlans.length + tradeExecutions.length + tradeFees.length + savedAnalyses.length + savedTimeAssistedSnapshots.length + tradeDiscipline.length,
