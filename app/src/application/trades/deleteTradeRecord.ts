...
export type DeleteTradeRecordResult =
  | { readonly ok: true; readonly tradeId: TradeId; readonly source: TradeSource; readonly removed: Readonly<{ plans: number; executions: number; fees: number }> }
 ...
 *
 * Owns only the removal of one trade record and its own plans, executions and
 * fees by the trade's canonical stable id inside one atomic write: the trade is
...
export async function deleteTradeRecord(db: KairosDatabase, tradeId: TradeId): Promise<DeleteTradeRecordResult> {
  try {
    const outcome = await runKairosAtomicWrite(db, ['trades', 'tradePlans', 'tradeExecutions', 'tradeFees'], async ({ repositories }) => {
      ...
      return { source: trade.source, removed: Object.freeze({ plans: plans.length, executions: executions.length, fees: fees.length }) };
    });
