...
export type CommitTradeImportResult =
  | { readonly ok: true; readonly added: Readonly<{ trades: number; plans: number; executions: number; fees: number }>; readonly skipped: number }
  | { readonly ok: false; readonly type: 'identity-conflict'; readonly reason: 'child-id-present' }
 ...
 * not on this device, with their own plans, executions and fees. Nothing is
...
export async function prepareTradeImport(db: KairosDatabase, serializedBackup: string): Promise<PrepareTradeImportResult> {
  ...
  try {
    const selection = await db.transaction('r', [db.trades, db.tradePlans, db.tradeExecutions, db.tradeFees], async () => {
      ...
      const [planKeys, executionKeys, feeKeys] = await Promise.all([db.tradePlans.toCollection().primaryKeys(), db.tradeExecutions.toCollection().primaryKeys(), db.tradeFees.toCollection().primaryKeys()]);
      ...
      return { trades, plans, executions, fees, conflict, alreadyPresent: incoming.payload.trades.length - trades.length };
    });
    ...
    return { ok: true, import: Object.freeze({ preview, trades: Object.freeze(selection.trades), plans: Object.freeze(selection.plans), executions: Object.freeze(selection.executions), fees: Object.freeze(selection.fees) }) };
  } catch {
    return { ok: false, type: 'storage-error', reason: 'import-prepare-failed' };
  }
}
...
export async function commitTradeImport(db: KairosDatabase, prepared: PreparedTradeImport): Promise<CommitTradeImportResult> {
  try {
    return await runKairosAtomicWrite(db, ['trades', 'tradePlans', 'tradeExecutions', 'tradeFees'], async ({ repositories }): Promise<CommitTradeImportResult> => {
      const write = { trades: [] as DatabaseTradeRecord[], plans: [] as DatabaseTradePlanRecord[], executions: [] as DatabaseTradeExecutionRecord[], fees: [] as DatabaseTradeFeeRecord[] };
      let skipped = 0;
      ...
      return { ok: true, added: Object.freeze({ trades: write.trades.length, plans: write.plans.length, executions: write.executions.length, fees: write.fees.length }), skipped };
    });
  } catch {
