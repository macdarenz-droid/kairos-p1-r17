...
function facts(entry: SavedRecordIndexEntry): string {
  if (entry.kind === 'analysis') return `${entry.drawingCount === 1 ? '1 line' : `${entry.drawingCount} lines`}${entry.riskRewardCount > 0 ? ` · ${entry.riskRewardCount === 1 ? '1 risk/reward' : `${entry.riskRewardCount} risk/rewards`}` : ''}`;
