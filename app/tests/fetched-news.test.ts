...
describe('refreshing the saved calendar', () => {
  ...
  it('keeps at most 5,000 fetched rows: the oldest go and the coverage no longer claims them', async () => {
    ...
    const oldTyped: EconomicEventRecord = { ...typedRow, id: economicEventId('typed', 'old'), startsAt: '2024-12-31T00:00:00.000Z' };
    await db.economicEvents.bulkPut([...fed, typedRow, oldTyped]);
    ...
    expect((await db.economicEvents.count()) - 2).toBe(5_000);
    ...
    expect(await db.economicEvents.get(oldTyped.id)).toBeDefined();
