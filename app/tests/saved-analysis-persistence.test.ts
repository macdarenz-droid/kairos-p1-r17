...
describe('P20.2 Saved Analysis persistence foundation', () => {
  it('persists Saved Analysis through its repository with clone-safe reads', async () => {
    ...
    await expect(assertKairosDatabaseIntegrity(db)).resolves.toMatchObject({ ok: true, schemaVersion: 6, savedAnalysisRecordCount: 1 });
    db.close();
  });
  ...
  it('round-trips Saved Analysis through current backup V4', () => {
    ...
    expect(parsed).toMatchObject({ formatVersion: 5, databaseSchemaVersion: 6, recordCounts: { savedAnalyses: 1, savedTimeAssistedSnapshots: 0, tradeDiscipline: 0, total: 1 } });
