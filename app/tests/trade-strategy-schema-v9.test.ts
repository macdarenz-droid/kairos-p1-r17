...
describe('T-040c schema v9: a trade can carry its strategy', () => {
  ...
  it('exports format 8 with schema 9 and the mark, which round-trips', async () => {
    ...
    expect(snapshot).toMatchObject({ formatVersion: 8, databaseSchemaVersion: 9 });
  ...
  it('reads a format 7 backup as format 8, keeping its record, and restores it', async () => {
    ...
    expect(parsed).toMatchObject({ formatVersion: 8, databaseSchemaVersion: 9, recordCounts: { tradeDiscipline: 1 } });
  ...
  it('refuses a mark in format 7, a format 8 header naming schema 8, and format 9', () => {
    ...
    expect(() => parseKairosBackup(JSON.stringify({ ...header, formatVersion: 9, databaseSchemaVersion: 10, recordCounts: counts, payload: payload(marked) }))).toThrow(expect.objectContaining({ code: 'UNSUPPORTED_FORMAT_VERSION' }));
  });
});
